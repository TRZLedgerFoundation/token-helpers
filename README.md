...
A set of helpers and wrappers around multiple Trezoa token libraries that enable developers to create token accounts with one-liners, supporting different token standards including Token Program (TokenKeg), Token-2022, and Token-ACL.
...
```bash
npm install @trezoa/token-helpers
# or
pnpm add @trezoa/token-helpers
```
...
```typescript
import { createAndConfirmAssociatedTokenAccount } from '@trezoa/token-helpers';
...
```typescript
import { createAssociatedTokenAccount } from '@trezoa/token-helpers';
...
```typescript
import { createAssociatedTokenAccountInstructions } from '@trezoa/token-helpers';
